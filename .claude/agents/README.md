...
| **devops-reviewer**        | Workflows, release path, Changesets invariants, images, compose, exposure, secrets                                 | `.github/**`, `**/Dockerfile`, `docker-compose*.yml`, nginx config, `.changeset/config.json`, `docs/{OPERATIONS,RELEASING}.md` | Read, Grep, Glob, Bash / sonnet                                              |

Every agent treats [`docs/PRODUCT.md`](../../docs/PRODUCT.md) and ADR-0019 as
winning over any standards document that conflicts with them.

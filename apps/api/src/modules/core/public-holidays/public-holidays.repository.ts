...
/**
 * Public holidays repository — the **data-access layer** (ADR-0008). It is the
...
export class PublicHolidaysRepository {
  ...
  private active(where: Prisma.PublicHolidayWhereInput = {}): Prisma.PublicHolidayWhereInput {
    return { ...where, deletedAt: null };
  }

  async create(
    data: Prisma.PublicHolidayUncheckedCreateInput,
    db: Prisma.TransactionClient = this.prisma,
  ): Promise<PublicHoliday> {
    return db.publicHoliday.create({ data });
  }

  /**
   * Insert several rows in one statement, skipping any that an active row
   * already holds (`ON CONFLICT DO NOTHING`, which covers the partial unique
   * index), and return the rows actually inserted. So concurrent imports of a
   * year both succeed and add each date once.
   */
  async createManyMissing(
    data: Prisma.PublicHolidayCreateManyInput[],
    db: Prisma.TransactionClient = this.prisma,
  ): Promise<PublicHoliday[]> {
    return db.publicHoliday.createManyAndReturn({ data, skipDuplicates: true });
  }
  ...
  async findAllActive(
    params: {
      ...
      orderBy: Prisma.PublicHolidayOrderByWithRelationInput[];
    },
    db: Prisma.TransactionClient = this.prisma,
  ): Promise<PublicHoliday[]> {
    return db.publicHoliday.findMany({ where: this.active(params.where), orderBy: params.orderBy });
  }
  ...
  async findActiveById(
    ...
    return db.publicHoliday.findFirst({ where: this.active({ id }) });
  }
  ...
  async findManyActive(
    ...
    if (params.cursor) {
      ...
      const anchor = await db.publicHoliday.findFirst({
  ...
  async updateIfVersionMatches(
    ...
    data: Prisma.PublicHolidayUpdateManyMutationInput,
    db: Prisma.TransactionClient = this.prisma,
  ): Promise<number> {
    const result = await db.publicHoliday.updateMany({

...
/**
 * Time adjustments repository — the **data-access layer** (ADR-0008). It is the
...
export class TimeAdjustmentsRepository {
  ...
  private active(where: Prisma.TimeAdjustmentWhereInput = {}): Prisma.TimeAdjustmentWhereInput {
    return { ...where, deletedAt: null };
  }

  async create(
    data: Prisma.TimeAdjustmentUncheckedCreateInput,
  ...
  async findActiveById(
    id: string,
    db: Prisma.TransactionClient = this.prisma,
  ): Promise<TimeAdjustment | null> {
    return db.timeAdjustment.findFirst({ where: this.active({ id }) });
  }
  ...
  async findById(
    ...
    return db.timeAdjustment.findUnique({ where: { id } });
  }
  ...
  async findManyActive(
    ...
    if (params.cursor) {
      ...
      const anchor = await db.timeAdjustment.findFirst({
  ...
  async updateIfVersionMatches(
    ...
    data: Prisma.TimeAdjustmentUpdateManyMutationInput,
    db: Prisma.TransactionClient = this.prisma,
  ): Promise<number> {
    const result = await db.timeAdjustment.updateMany({
  ...
  async softDelete(id: string, db: Prisma.TransactionClient = this.prisma): Promise<void> {
    await db.timeAdjustment.update({
      where: { id },

...
| 2   | At 1280px the footer's "of 37:30 target" wrapped onto three lines, and the aside stacked below the table (off-screen).            | The aside sits **beside** the table from a 56rem content width (a 1280px window) by a container query, and stacks below it under that, down to the 320 CSS px floor. The table was tightened to fit: 28px fields 3.25rem wide (wide enough for "08:00" at WCAG 1.4.12 text spacing, checked by Playwright), 13px table text, 4px numeric padding, and a day's warnings moved to a row of their own. The footer shows "of 37:30" on one short line ("target" is read by screen readers; the aside says it in words). A Playwright check proves the fit at 1280×800 and 1920×1080 with the widest table (the Converted column on and a row being edited): no page or table scroll. |
...
- Sidebar 224px, 16px narrower: the tools' names are short, and the 16px went to
  the week table at 1280px.
- `--width-aside` 15.5rem beside the week table at 1280px, `--width-aside-wide`
...
Measured with `pnpm --filter @repo/web build` (gzip), before (origin/main at
#64) and after:

| Asset                            | Before   | After    |
| -------------------------------- | -------- | -------- |
| CSS                              | 7.0 kB   | 9.0 kB   |
| JavaScript, all chunks           | ≈ 270 kB | ≈ 274 kB |
| Inter, latin woff2 (first visit) | —        | 48 kB    |

The initial JavaScript stays at about 90 kB, well within FRONTEND_QUALITY.md's
200 kB advisory budget. The other Inter subsets are built but downloaded only
by a page that uses their characters.

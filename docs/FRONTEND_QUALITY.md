...
(`e2e/hours-settings.spec.ts`, `e2e/hours-summary.spec.ts`,
`e2e/hours-week.spec.ts`) set 1280×800, run axe on every screen and reflow
their screens at 320 CSS px; the week journey is the first keyboard-only
journey through a whole task (entering a week, clearing a day and undoing it),
and also checks the week view with its aside at 1920×1080. Journeys that
depend on today set the browser's clock with `page.clock`.

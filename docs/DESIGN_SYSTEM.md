...
Light, dark and system. The preference is stored in `localStorage` and applied as
...
| **Toast**                                                    | One toaster, bottom-right, above dialogs (`--z-toast`), polite live region. Variants `success \| error \| info`. Success 4s, errors persist, **undo toasts 8s with a keyboard-reachable action**.                                                                 |
...
| **Skeleton**                                                 | Mirrors the final layout exactly so nothing shifts. First loads only, never refetches.                                                                                                                                                                            |
...
| **Inspector / side panel**                                   | Non-modal right panel over the detail area; Esc closes; the list behind stays operable. Not a modal, not an overlay drawer.                                                                                                                                       |
| **Badge, Breadcrumb, Tabs, Pagination, Search, Empty state** | As needed, to the same bar: token-driven, keyboard-complete, states covered, no colour-only meaning.                                                                                                                                                              |

...
| **Badge**        | A short, non-interactive status label: `text-xs`, 500 weight, `radius-sm`. Variants `default` (secondary surface), `outline` (an `input` border), `warning` (`warning` fill, for a warning with a ⚠ icon) and `info`. Its meaning is always in its text, never colour alone; an icon is `aria-hidden`. It wraps rather than widening a table cell. Not focusable.                                                                                                                                                                       |
